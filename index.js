...
function removeArtist(artists, number) {
  let newArtists = artists.slice()
  newArtists.splice(number, 1)
  return newArtists
}
...
function addArtist(artists, name, years, genre, nationality, bio) {
  ...
  let newArtist = {'name': name, 'years': years, 'genre': genre, 'nationality': nationality, 'bio': bio}
...
function artistInfo(artists, name){
  let selectedArtist = artists.filter(artist => artist['name'] == name) [0]
  return selectedArtist['bio']
}
...
function artistByCountry(artists, nationality){
  ...
  let filteredArtists = []
  for (let i in artists){
    if (artists[i]['nationality'] == nationalityList){
      filteredArtists.push(artists[i]['name'])
    }
  }
  return filteredArtists
